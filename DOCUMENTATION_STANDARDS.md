...
- name: order_id
  description: |
    Unique identifier for each customer order. Generated by the source system
    at order creation time. Used as primary key and for joining to order items
    and payment data.
  tests:
    - unique
    - not_null
...
- name: customer_id
  description: |
    Foreign key reference to dim_customers. Links orders to customer
    demographic and geographic information. Used for customer segmentation
